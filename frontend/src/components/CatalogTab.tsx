...
export const CatalogTab: React.FC<Props> = ({ catalog, currentWorkspaceId, onRefresh, onInsertDsl }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [copiedId, setCopiedId] = useState<string | null>(null);
  const [insertedId, setInsertedId] = useState<string | null>(null);
  const [isRefreshing, setIsRefreshing] = useState(false);

  // Restrict to components in the current workspace
  ...
  const displayedItems = latestCatalogItems.filter((item) =>
    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-slate-950 text-slate-200">
      ...
      <div className="px-3 py-1.5 bg-slate-900/20 border-b border-slate-800/60 flex items-center justify-between text-[11px] text-slate-400 shrink-0">
        <span className="flex items-center gap-1.5">
          ...
          <span>{displayedItems.length} workspace component{displayedItems.length === 1 ? '' : 's'}</span>
        </span>
        ...
        {displayedItems.length === 0 ? (
          <div className="text-center py-12 text-slate-500">
            ...
            <p className="font-semibold text-xs text-slate-400">
              {workspaceCatalogItems.length === 0 ? 'No components published in this workspace yet' : 'No matching components found'}
            </p>
            <p className="text-[11px] text-slate-500 mt-1 max-w-xs mx-auto">
              {workspaceCatalogItems.length === 0
                ? 'Publish this workspace to register its components and software systems in the enterprise catalog.'
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    <span className="px-1.5 py-0.2 text-[9px] uppercase font-bold tracking-wider bg-emerald-500/15 text-emerald-400 rounded border border-emerald-500/25">
                      Latest
                    </span>
                  </div>
                  {item.description && (
